...
function EditCard(props) {
  let [, setCard] = useState({})

  let { register, handleSubmit, setValue, formState: { errors } } = useForm();
  let history = useHistory();

  let nameRef = register("bizName", { required: true, minLength: 2 }),
    descRef = register("bizDescription", { required: true, minLength: 2 }),
    addressRef = register("bizAddress", { required: true, minLength: 2 }),
    phoneRef = register("bizPhone", { required: true, minLength: 2 }),
    imageRef = register("bizImage", { required: false });
